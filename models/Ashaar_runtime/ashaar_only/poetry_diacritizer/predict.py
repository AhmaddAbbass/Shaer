...
class GeneralTrainer(Trainer):
    ...
    def set_device(self):
        cfg_device = self.config.get("device")
        ...
        # Keep config in sync with the effective device
    ...
    def load_model(self, model_path: str = None, load_optimizer: bool = True):
      ...
      # Checkpoint created with older PyTorch; allow full unpickling.
      saved_model = torch.load(last_model_path, map_location=device, weights_only=False)
