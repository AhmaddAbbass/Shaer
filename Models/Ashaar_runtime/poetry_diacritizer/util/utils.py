...
def plot_alignment(alignment: torch.Tensor, path: str, global_step: Any = 0):
    ...
    if plt is None:
        raise RuntimeError(
            "matplotlib is required for plot_alignment but is not installed."
        )
...
def display_attention(
    ...
    if plt is None:
        raise RuntimeError(
            "matplotlib is required for display_attention but is not installed."
        )
...
def plot_multi_head(model, path, global_step):
    if plt is None:
        raise RuntimeError(
            "matplotlib is required for plot_multi_head but is not installed."
        )
