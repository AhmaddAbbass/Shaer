...
def _restore_checkpoint(model, ckpt_prefix):
    """Restore a TensorFlow checkpoint into a Keras model in TF 2 / Keras 3."""
    checkpoint = tf.train.Checkpoint(model=model)
    status = checkpoint.restore(ckpt_prefix)
    # Allow missing optimizer or other non-model objects
    try:
...
class BaitAnalysis:
    def __init__(self, abs_path=".", use_cbhg=True):
        # Resolve base paths for config (test.yml) and models (deep-learning-models)
        package_dir = os.path.dirname(__file__)
        project_root = os.path.dirname(package_dir)

        if abs_path == ".":
            # Prefer test.yml in the project root, but fall back to package_dir.
            if os.path.isfile(os.path.join(project_root, "test.yml")):
                config_yml = os.path.join(project_root, "test.yml")
            else:
                config_yml = os.path.join(package_dir, "test.yml")

            # Prefer deep-learning-models in the project root, otherwise use package_dir.
            if os.path.isdir(os.path.join(project_root, "deep-learning-models")):
                base_path = project_root
            elif os.path.isdir(os.path.join(package_dir, "deep-learning-models")):
                base_path = package_dir
            else:
                base_path = project_root
        else:
            base_path = abs_path
            config_yml = os.path.join(base_path, "test.yml")
        ...
        # Ensure pretrained models are available without re-downloading if possible
        models_dir = os.path.join(self.base_path, "deep-learning-models")
        zip_candidates = [
            os.path.join(self.base_path, "deep-learning-models.zip"),
            os.path.join(package_dir, "deep-learning-models.zip"),
        ]

        if not os.path.isdir(models_dir):
            existing_zip = next((z for z in zip_candidates if os.path.isfile(z)), None)
            if existing_zip is not None:
                print(f"Found existing {os.path.basename(existing_zip)}, extracting ...")
                gdown.extractall(existing_zip)
            else:
        ...
        _restore_checkpoint(
            self.METERS_MODEL,
            os.path.join(
                self.base_path, "deep-learning-models", "meters_model", "cp.ckpt"
            ),
        )
        ...
        self.BAITS_EMBEDDINGS = load_from_disk(
            os.path.join(self.base_path, "deep-learning-models", "baits_embeddings")
        )
        ...
        _restore_checkpoint(
            ...
            os.path.join(
                self.base_path,
                "deep-learning-models",
                "era_classification_models_shorter_context",
                "cp.ckpt",
            ),
        )

        self.ERA_TOKENIZER = SentencePieceTokenizer()
        ...
        _restore_checkpoint(
            ...
            os.path.join(
                self.base_path,
                "deep-learning-models",
                "theme_classification_model",
                "cp.ckpt",
            ),
        )

        self.THEME_TOKENIZER = SentencePieceTokenizer()
