...
def score_meaning_fit(description: str, generated_poem: str, prompt_file: str, cache_dir: str):
    cfg = load_yaml(prompt_file)
    client = JudgeClient(cache_dir=cache_dir, cache_namespace="meaning_fit")
    ...
    parsed = extract_judge_json(raw, score_key=cfg.get("score_key", "fit_score"))
    score_key = cfg.get("score_key", "fit_score")
    return {
...
def make_meaning_fit_reward(run_dir: str, prompt_file: str, cache_dir: str, recorder=None):
    debug_path = f"{run_dir}/reward_meaning_fit_debug.jsonl"

    def reward_fn(prompts=None, completions=None, description=None, dataset_split=None, **kwargs):
        ...
        for i, completion in enumerate(completions):
            ...
            out = score_meaning_fit(desc, poem, prompt_file=prompt_file, cache_dir=cache_dir)
    ...
    reward_fn.__name__ = "meaning_fit"
    return reward_fn
