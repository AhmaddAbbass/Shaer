...
- meter_reward(completions, poem_meter=None, ...) -> list[float]   (optional batch helper)
"""
...
def _lazy_import_tf():
    try:
        ...
        raise ImportError(
            "tensorflow is required for BiLSTM meter classifier. Install it first."
        ) from exc
    return tf
...
def _load_bilstm_assets(
    ...
    if _BILSTM_MODEL is None or _LABEL_ENCODER is None or not _STOI or _MAX_LEN <= 0:
        tf = _lazy_import_tf()
...
# ---------------------------------------------------------------------------
# Optional GRPO-friendly batch wrapper (classifier-only reward)
# ---------------------------------------------------------------------------
