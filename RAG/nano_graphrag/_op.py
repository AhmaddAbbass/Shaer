...
async def _merge_nodes_then_upsert(
    ...
    # NOTE: we also aggregate per-chunk metadata (book_id, title, author)
    # from addon_params["chunk_meta_map"], so we can tag each entity with
    # its originating book and later build a per-book kg_id.
    addon_params = global_config.get("addon_params", {}) or {}
    chunk_meta_map = addon_params.get("chunk_meta_map", {}) or {}
    ...
    if already_node is not None:
        ...
        # reuse any existing metadata on the node if present
        if already_node.get("book_id"):
            book_ids.add(already_node["book_id"])
        if already_node.get("book_title_ar"):
            book_title_ars.add(already_node["book_title_ar"])
        if already_node.get("author_ar"):
            author_ars.add(already_node["author_ar"])
        if already_node.get("author_en"):
            author_ens.add(already_node["author_en"])

    # collect metadata from all chunk_ids that mentioned this entity
    for dp in nodes_data:
        src_id = dp.get("source_id")
        if not src_id:
            continue
        meta = chunk_meta_map.get(src_id)
        if not meta:
            continue
        if meta.get("book_id"):
            book_ids.add(meta["book_id"])
        if meta.get("book_title_ar"):
            book_title_ars.add(meta["book_title_ar"])
        if meta.get("author_ar"):
            author_ars.add(meta["author_ar"])
        if meta.get("author_en"):
            author_ens.add(meta["author_en"])

    def _pick_one(values: set):
        # helper: pick a stable single value from a set, or None
        if not values:
            return None
        if len(values) == 1:
    ...
    book_id = _pick_one(book_ids)
    book_title_ar = _pick_one(book_title_ars)
    author_ar = _pick_one(author_ars)
    author_en = _pick_one(author_ens)

    # choose a kg_id that is specific to (entity, book)
    if book_id is not None:
        kg_id = f"{entity_name}::book::{book_id}"
    else:
        kg_id = entity_name
    ...
    # attach book-level metadata + kg_id to the node so it is available
    # both in the graph and when we later build vector DB entries.
    if book_id is not None:
        node_data["book_id"] = book_id
    if book_title_ar is not None:
        node_data["book_title_ar"] = book_title_ar
    if author_ar is not None:
        node_data["author_ar"] = author_ar
    if author_en is not None:
        node_data["author_en"] = author_en
    node_data["kg_id"] = kg_id
...
async def extract_entities(
    ...
    if entity_vdb is not None:
        # NOTE: use kg_id (entity+book) when available so that entities from
        # different books get distinct vector entries. We also pass through
        # book metadata so Neo4jVectorStorage can index/filter on it.
        data_for_vdb = {
