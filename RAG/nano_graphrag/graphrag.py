...
class GraphRAG:
    ...
    def __post_init__(self):
        ...
        self.entities_vdb = (
            self.vector_db_storage_cls(
                ...
                meta_fields={"entity_name", "book_id", "book_title_ar", "author_ar","author_en"},
    ...
    async def ainsert(self, string_or_strings):
        ...
        try:
            ...
            # NOTE: if external code provided per-document metadata in addon_params["doc_meta_map"],
            # we propagate it down to a per-chunk mapping. This will later be used by the
            # entity extraction code (via global_config["addon_params"]["chunk_meta_map"])
            # so that each entity can be tagged with its book/author information.
            doc_meta_map = self.addon_params.get("doc_meta_map", {}) or {}
            if doc_meta_map:
                chunk_meta_map = {}
                for chunk_id, chunk in inserting_chunks.items():
                    full_doc_id = chunk.get("full_doc_id")
                    if full_doc_id and full_doc_id in doc_meta_map:
                        chunk_meta_map[chunk_id] = doc_meta_map[full_doc_id]
                self.addon_params["chunk_meta_map"] = chunk_meta_map
